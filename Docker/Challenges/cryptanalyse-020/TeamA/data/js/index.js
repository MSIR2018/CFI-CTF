...
mhnUI = {
    pattern: "",
    setup: function() {
        this.lock(), setInterval("mhnUI.datetime()", 1e3)
    },
    lock: function() {
        mhnUI.page.hide(), pattern = new PatternLock(".mhn-lock", {
            margin: 15
        }), $(".mhn-lock-title").html($(".mhn-lock-title").data("title")), pattern.removeAfterCreate(atob("OTU4NzQxMjM2"), function() { $(".patt-holder").addClass("patt-success"), setTimeout(function() {
            pattern.reset(), alert(atob("RulsaWNpdGF0aW9u"));
        }, 1e3)
        }, function() { $(".patt-holder").removeClass("patt-success"), setTimeout(function() {
            pattern.reset()
        }, 2e3)
    ...
    datetime: function() {
        var t = new Array("Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi"),
            e = new Array("Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Decembre"),
            n = new Date,
            i = n.getYear();
        1e3 > i && (i += 1900);
        var a = n.getDay(),
        ...
        var h = n.getHours(),
            ...
            u = n.getSeconds(),
            l = "";
        //h >= 12 && (l = "PM"), h > 12 && (h -= 12), 0 == h && (h = 12),
        9 >= c && (c = "0" + c), 9 >= u && (u = "0" + u), $(".mhn-ui-date-time .mhn-ui-day").text(t[a]), $(".mhn-ui-date-time .mhn-ui-date").text(s + " " + e[o] + " " + i), $(".mhn-ui-date-time .mhn-ui-time").text(h + ":" + c + " " + l), $(".mhn-ui-app-time").text(h + ":" + c + ":" + u)
    },
    page: {
        hide: function(t) {
